...
def main() -> None:
    ...
    if not args.all:
        ...
        if run_pages:
            ...
            print(f"[timing] Pages: {elapsed:.1f}s ({total} pages, {args.workers} workers)")
        ...
        if run_cards:
            ...
            elapsed = time.perf_counter() - t0
            print(f"[timing] Cards: {elapsed:.1f}s ({len(all_page_paths)} pages, {len(card_paths)} cards, {args.workers} workers)")
        ...
        if run_pages:
            ...
            if args.workers > 1 and len(pdf_files) > 1:
                with tqdm(total=len(pdf_files), unit="vol") as bar:
                    with ProcessPoolExecutor(max_workers=args.workers) as pool:
                        futures = {
                            pool.submit(extract_pages_from_pdf, pdf_path, config, args.force): pdf_path
            ...
            elapsed = time.perf_counter() - t0
            print(f"[timing] Pages: {elapsed:.1f}s ({len(pdf_files)} volumes, {args.workers} workers)")
        ...
        if run_cards:
            ...
            if args.workers > 1 and len(page_images) > 1:
                with tqdm(total=len(page_images), unit="page") as bar:
                    with ProcessPoolExecutor(max_workers=args.workers) as pool:
                        futures = {
                            pool.submit(
                                extract_cards_from_page, page_path, config, args.force,
                                None, debug_base / page_path.stem if debug_base else None,
                            ): page_path
            ...
            elapsed = time.perf_counter() - t0
            print(f"[timing] Cards: {elapsed:.1f}s ({len(page_images)} pages, {args.workers} workers)")
